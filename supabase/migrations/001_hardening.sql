-- CompatKink — Migration 001: Hardening (idempotent)
-- Ejecutar en SQL Editor si el proyecto ya existía antes del hardening unificado.
-- Debe quedar equivalente a supabase/schema.sql (fuente de verdad).
--
-- Defaults canónicos:
--   expires_at default / create TTL = 48 hours
--   invite_attempts  = rate limit de canje de código
--   rpc_rate_limits  = rate limit genérico (create / token / submit)
...
-- 2. invite_attempts (guest brute-force)
...
-- 3. rpc_rate_limits (generic buckets)
...
-- 6. RPCs (mismas firmas y límites que schema.sql)
